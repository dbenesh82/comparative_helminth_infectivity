...
```
```{r}
# remove those potentially undercounted
dat <- filter(dat, !Detect_Problem) 
```
...
dat <- mutate(dat, log_ws = log(imp_biovolume), log_hm = log(host_bm))%>%
...
mod1 <- MCMCglmm(Recovery_Rate ~ log_dpi + nh_fac*to_int_def + log_ws + log_hm, 
                 ...
                 data = filter(dat, !is.na(imp_biovolume), !is.na(host_bm)),
                 verbose = F)
...
#                         rcov = ~us(trait):units, # residual var-covar unstructured
#                         nitt = nit, thin = 20,
#                         data = filter(dat, !is.na(imp_biovolume)), 
...
```{r}
tx <- filter(dat, !is.na(imp_biovolume), !is.na(host_bm))
tx2 <- tx
tx2.1 <- tx
tx3 <- tx
# tx4 <- tx
...
ggsave(f1, filename = "figs/fig1_imp_uc.png", width = 6, height = 4)
# ggsave(f1, filename = "figs/fig1_imp_uc.svg", width = 6, height = 4)
```
...
```{r}
st <- filter(dat, !is.na(imp_biovolume), !is.na(host_bm))%>%
...
startc <- list(R = as.numeric(lme4::VarCorr(m2_rs0)["obs"][[1]]),
               G = list(G1 = rs),
               QUASI = FALSE)
mod_rs_fe <- MCMCglmm(cbind(succeeded, failed) ~ log_dpi + nh_fac * to_int_def + log_ws + log_hm, 
                 random = ~ us(1+log_dpi):study_rs,
                 prior = priorRS,
                 start = startc,
                 nitt = nit, thin = 40, burnin = bi,
...
startc <- list(R = as.numeric(lme4::VarCorr(m2_rs0)["obs"][[1]]),
               G = list(G1 = rs,
                        G2 = rs[1]),
               QUASI = FALSE)
modp_rs_fe <- MCMCglmm(cbind(succeeded, failed) ~ log_dpi + nh_fac * to_int_def + log_ws + log_hm, 
                 random = ~ us(1+log_dpi):study_rs + tree_tips,
                 prior = priorRSP,
                 start = startc,
                 nitt = nit, thin = 40, burnin = bi,
...
modpt <- MCMCglmm(cbind(succeeded, failed) ~ log_dpi + nh_fac * to_int_def + log_ws + log_hm, 
                 random = ~ us(1+log_dpi):study_rs +
...
```{r}
f_avg <- filter(dat, !is.na(imp_biovolume), !is.na(host_bm))%>%
  group_by(parasite_phylum, parasite_order, Stage)%>%
  summarise(Recovery_Rate = median(Recovery_Rate))%>%
  arrange(Recovery_Rate,Stage)
ggplot(filter(dat, !is.na(imp_biovolume), !is.na(host_bm))%>%
...
```{r}
qplot(summary(mod_rs_fe)$solutions[,1], summary(modp_rs_fe)$solutions[,1]) +
  geom_abline(slope = 1, intercept = 0) +
  geom_label(aes(label = names(summary(mod_rs_fe)$solutions[,1]))) +
  geom_point() +
  labs(x = "Parameter estimates, non-phylogenetic model", y = "Parameter estimates, phylogenetic model")
```
...
ggsave(f2, filename = "figs/fig2_imp_uc.png", width = 4, height = 4)
# ggsave(f2, filename = "figs/fig2_imp_uc.svg", width = 4, height = 4) # remove boxes under legend
```
...
f3
# ggsave(f3, filename = "figs/fig3_imp_uc.png", width = 6, height = 4.5)
```
...
f3 <- ggplot(dat,
  ...
  facet_wrap(~to_int_def)
f3
# ggsave(f3, filename = "figs/fig3_imp_uc.png", width = 8, height = 4)
# ggsave(f3, filename = "figs/fig3_imp_uc.svg", width = 8, height = 4)
```
...
f3 <- ggplot(datx,
  ...
  theme(panel.grid.minor = element_blank()) 
f3
ggsave(f3, filename = "figs/fig3_imp_uc.png", width = 6, height = 4)
# ggsave(f3, filename = "figs/fig3_imp_uc.svg", width = 6, height = 4)
```
...
f4 <- ggplot(datx,
  ...
  # facet_wrap(~to_int_def)
f4
# ggsave(f4, filename = "figs/fig4_imp_uc.png", width = 6, height = 4.5)
```
...
f4 <- ggplot(datx,
  ...
  facet_wrap(~to_int_def)
f4
# ggsave(f4, filename = "figs/fig4_imp_uc.png", width = 8, height = 4)
# ggsave(f4, filename = "figs/fig4_imp_uc.svg", width = 8, height = 4)
```
...
f4 <- ggplot(datx,
  ...
  theme(panel.grid.minor = element_blank()) 
f4
ggsave(f4, filename = "figs/fig4_imp_uc.png", width = 6, height = 4)
# ggsave(f4, filename = "figs/fig4_imp_uc.svg", width = 6, height = 4)
```
...
f5a <- ggplot(dose_per_study,
  ...
  labs(x = "Host in life cycle exposed", y = "Dose",
       shape = "")
f5a
# ggsave(f5a, filename = "figs/fig5_imp.png", width = 4, height = 4)
# ggsave(f5a, filename = "figs/fig5_imp.svg", width = 4, height = 4) # remove boxes under legend
```
...
f6 <- cowplot::plot_grid(f6a2, f6b2, f6c2,
                         align = "hv", nrow = 1)
ggsave(f6, filename = "figs/fig5_imp_uc.png", width = 12, height = 4)
# ggsave(f6, filename = "figs/fig5_imp_uc.svg", width = 8, height = 8)
```

Thus, experimenters usually use higher doses in situations where lower infection rates are expected.

```{r}
save.image(file = "ER_analysis_aftermod_imp_uc.RData")
```
